# AgentFlow Documentation Index

## Framework Core

- [Governance](../GOVERNANCE.md)
- [Operating Contract](../AGENTFLOW.md)
- [Delivery Lifecycle](../DELIVERY-LIFECYCLE.md)
- [AI Execution Rules](../AI-EXECUTION-RULES.md)
- [Documentation Policy](../DOCUMENTATION-POLICY.md)
- [Framework Configuration](../FRAMEWORK-CONFIG.md)
- [Bootstrap Procedure](../BOOTSTRAP-PROCEDURE.md)
- [Adoption Guide](../ADOPTION-GUIDE.md)

## Templates

See [templates](../templates/):

- Requirement
- Development Transfer
- Architecture Decision
- Agent Task Contract
- Evidence Bundle
- Independent Review
- Executable Task
- Release Record
- Project Adapter
- Bootstrap Report

## Framework development context

- [Project Context Handoff](PROJECT-CONTEXT-HANDOFF.md) — current state for a fresh chat/agent
- [Origin and Design Intent](design/ORIGIN-AND-DESIGN-INTENT.md) — original problem, Conversation Independence Principle and Cold Start Test

## Assessment / audit

- [Maturity Assessment — RO](reference/MATURITY-ASSESSMENT-RO.md)
- [External Audit Template](audit/EXTERNAL-AUDIT-TEMPLATE.md)
- [Usage Guide — RO](reference/USAGE-GUIDE-RO.md)

## Research

- [Market Landscape](research/MARKET-LANDSCAPE.md)

## Reading rule

Do not load all documents by default.

For execution, start with the current task, the AI execution rules, affected code/configuration, and only the relevant canonical sections.

For framework development from a new conversation, start with `PROJECT-CONTEXT-HANDOFF.md`.
