# AgentFlow Kit Manifest

Status: `CANONICAL INVENTORY`  
Framework version: `1.2.0`

This file is the single normative inventory of the AgentFlow v1.2.0 repository kit.

## Core / release

- VERSION
- README.md
- CHANGELOG.md
- COMPATIBILITY.md
- KIT-MANIFEST.md
- GOVERNANCE.md
- AGENTFLOW.md
- DELIVERY-LIFECYCLE.md
- AI-EXECUTION-RULES.md
- ARTIFACT-TRACEABILITY.md
- METRICS.md
- FRAMEWORK-CONFIG.md
- BOOTSTRAP-PROCEDURE.md
- DOCUMENTATION-POLICY.md
- ADOPTION-GUIDE.md
- agentflow.config.example.yaml

## Templates

- templates/REQUIREMENT.md
- templates/DEVELOPMENT-TRANSFER.md
- templates/ARCHITECTURE-DECISION.md
- templates/DEVELOPMENT-ANALYSIS.md
- templates/AGENT-TASK-CONTRACT.md
- templates/EVIDENCE-BUNDLE.md
- templates/INDEPENDENT-REVIEW.md
- templates/CANDIDATE-MANIFEST.md
- templates/EXECUTABLE-TASK.md — DEPRECATED compatibility view
- templates/RELEASE-RECORD.md
- templates/PROJECT-ADAPTER.md
- templates/BOOTSTRAP-REPORT.md

## Example

- examples/EXAMPLE-FLOW.md

## Framework development / reference

- docs/INDEX.md
- docs/PROJECT-CONTEXT-HANDOFF.md
- docs/design/ORIGIN-AND-DESIGN-INTENT.md
- docs/reference/CORE-VERSION-MATRIX.md
- docs/reference/USAGE-GUIDE-RO.md
- docs/reference/MATURITY-ASSESSMENT-RO.md
- docs/audit/EXTERNAL-AUDIT-TEMPLATE.md
- docs/audit/reports/AF-AUDIT-2026-09-23-01.md
- docs/audit/reports/AF-AUDIT-2026-09-23-01-REMEDIATION-v1.2.0.md
- docs/research/MARKET-LANDSCAPE.md

## Inventory rule

A shipped framework file must be added here in the same change that introduces it.

A repository file omitted from this manifest is not part of the canonical distributable kit until explicitly listed.
